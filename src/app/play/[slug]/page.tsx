...
export default function PlayPage({ params }: Props) {
  ...
  const [gameData, setGameData] = useState<{ childName: string; themeColor: string; wishes: Wish[] } | null>(null)
  ...
  useEffect(() => {
    async function fetchGame() {
      ...
      const mappedWishes: Wish[] = wishes.map((w: any, i: number) => ({
        from: w.from_name,
        av: w.avatar,
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#080010', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16 }}>
      ...
      <p style={{ fontFamily: "'Racing Sans One',cursive", color: '#d8b4fe', fontSize: '1.5rem', letterSpacing: 2 }}>Loading game...</p>
  ...
  if (notFound) return (
    <div style={{ minHeight: '100vh', background: '#080010', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 16, padding: 24, textAlign: 'center' }}>
      ...
      <h1 style={{ fontFamily: "'Racing Sans One',cursive", color: '#f87171', fontSize: '2rem' }}>Game not found!</h1>
      <p style={{ fontFamily: "'Boogaloo',cursive", color: 'rgba(255,255,255,0.6)', fontSize: '1.2rem' }}>This birthday game link may have expired or the URL is wrong.</p>
